...
TrafficLogStats = list[Tuple[Literal["r", "w", "c"], int]]
PatchMethods = tuple[
...
def reserial(
    ...
    if log_stats is not None and len(log_stats) > 0:
...
def get_traffic_log_stats(mode: Mode, log_path: Path, test_name: str) -> TrafficLogStats | None:
    if mode == Mode.INVALID:
        msg = "Choose one of 'replay' or 'record', not both"
        raise ValueError(msg)

    log: TrafficLogStats | None = []
...
def get_patched_methods(mode: Mode, log: TrafficLog, log_stats: TrafficLogStats | None) -> PatchMethods:
...
def get_replay_methods(log: TrafficLog, log_stats: TrafficLogStats | None) -> PatchMethods:
    ...
    def replay_write(
        ...
        if log_stats is not None and log_stats[0][0] == "w":
            del log_stats[0]
        elif log_stats is not None:
            msg = (
                "Written data does not match recorded data: "
