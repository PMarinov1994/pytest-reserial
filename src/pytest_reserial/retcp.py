...
class TestTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle_replay(self: TestTCPHandler) -> bool:
        ...
        while len(self.log_stats) > 0:
            ...
            while len(self.log_stats) > 0 and self.log_stats[0][0] == "r":
                response_info = self.log_stats.pop(0)
                size = response_info[1]
...
def retcp(monkeypatch: pytest.MonkeyPatch,
          request: pytest.FixtureRequest) -> Generator[int, Any, Any]:
    ...
    """
    port_param = int(str(request.param))
    ...
    if log_stats is None:
        msg = f"No stats recorded for test: {test_name}"
    ...
    if mode == Mode.REPLAY:
        ...
        port_param = server.server_address[1]
        ...
        msg = "Invalid mode. Only --record and --replay are possible"
        pytest.fail(msg)

    yield port_param

    if mode == Mode.REPLAY and tests_thread and watcher_thread:
        tests_thread.join(timeout=10)
